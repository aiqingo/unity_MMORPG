...
namespace GameServer.Services
{
    class GuildService:Singleton<GuildService>
    {
        ...
        private void OnGuildAdmin(NetConnection<NetSession> sender, GuildAdminRequest message)
        {
            ...
            var targer = SessionManager.Instance.GetSession(message.Target);
            if (targer!=null)
            {
                targer.Session.Response.guildAdmin = new GuildAdminResponse();
                targer.Session.Response.guildAdmin.Result = Result.Success;
                targer.Session.Response.guildAdmin.Command = message;
                targer.SendResponse();
            }
